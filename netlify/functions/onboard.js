...
Extra notes: ${extra_notes || "none"}
  ...
  const [welcomeRaw, brainRaw, promptsRaw, mapRaw, rulesRaw, checklistRaw, studyRaw] = await Promise.all([
    ...
    callClaude(`Create interactive study materials for ${name} at ${business_name} to reinforce their AI course.\n\n1. FLASHCARDS (8): Mix of AI vocabulary with plain-English definitions AND business-specific terms from their workflows. Make them memorable and practical.\n2. QUIZ (5 questions): Scenario-based multiple-choice questions testing real understanding of using AI at ${business_name}. Each has 4 choices, one correct answer (0-indexed), and a brief explanation.\n3. MATCH (5 pairs): Short terms (3-5 words) paired with brief definitions (6-12 words). For a tap-to-match game.\n\nReturn ONLY valid JSON with NO extra text before or after:\n{"flashcards":[{"term":"Business Brain","def":"A reusable prompt that gives AI context about your business — paste it at the start of every session."}],"quiz":[{"q":"You need to write a client proposal for ${business_name}. What do you do first?","choices":["Start typing in ChatGPT","Paste your Business Brain first","Use a generic template","Ask a colleague"],"correct":1,"explain":"Always paste your Business Brain first so AI has full context about your business and clients."}],"match":[{"term":"Context window","def":"Amount of text AI holds in one session"}]}\n\n${ctx}`, 2200),
  ]);
