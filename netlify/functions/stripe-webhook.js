...
Off limits / never use AI for: ${off_limits || "none specified"}
  ...
  const [welcomeRaw, brainRaw, promptsRaw, toolsRaw, mapRaw, rulesRaw, checklistRaw, flashcardsRaw, quizRaw, matchRaw] = await Promise.all([
    ...
    callClaude(`Create 10 flashcards to help ${name} master AI concepts relevant to ${business_name}. Each card: a short term on the front, a clear plain-English definition on the back — specific to their industry and workflows.\n\nReturn ONLY valid JSON array:\n[{"term":"System Prompt","def":"Instructions you give AI before the conversation starts — sets its role, tone, and limits."}]\n\n${ctx}`, 900),
    callClaude(`Create 8 multiple-choice quiz questions to test ${name}'s AI knowledge for ${business_name}. Each question has 4 choices with exactly one correct answer (0-indexed). Make questions practical — about prompting, tools, and AI usage for their specific business.\n\nReturn ONLY valid JSON array:\n[{"q":"What should you include in every prompt for best results?","choices":["Your name","Context about your business and goal","The date","Your email"],"correct":1,"explain":"Context helps AI give relevant, specific answers."}]\n\n${ctx}`, 900),
